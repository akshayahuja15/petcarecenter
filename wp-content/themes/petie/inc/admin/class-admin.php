...
class Givelove_Theme_Admin {
	...
	public function edit_menu_title_plugin( $args ) {
		...
		if ( $count > 0 ) {
			$args['menu_title'] = esc_html__( 'Install Plugins', 'petie' ) . ' <span class="update-plugins"><span class="update-count">' . $count . '</span></span>';
		}
	...
	public function dashboard_scrips( $hook ) {
		...
		if ( in_array( get_current_screen()->base, $listPage ) ) {
			...
			wp_enqueue_style( 'petie_dashboard_css', get_template_directory_uri() . '/inc/admin/css/admin.css' );
		}
	...
	public function add_settings_page() {
		...
		$args = array(
			'id'           => 'api_key',
			'title'        => esc_html__( 'Settings', 'petie' ),
			'object_types' => array( 'options-page' ),
			'option_key'   => 'opal-settings',
			'tab_group'    => 'opal_settings',
			'tab_title'    => esc_html__( 'API Key', 'petie' ),
		...
		$main_options->add_field( array(
			'name'        => esc_html__( 'Google API key', 'petie' ),
			'description' => sprintf( __( 'Please go to <a href="%s" target="_blank">Google developers page</a> to get API key.', 'petie' ), 'https://developers.google.com/maps/documentation/javascript/get-api-key' ),
			'id'          => 'api_key',
			'type'        => 'text',
		) );
	}
	...
	public function add_license_page() {
		...
		$cmb = new_cmb2_box( array(
			...
			'title'        => esc_html__( 'License', 'petie' ),
			'object_types' => array( 'options-page' ),
			'option_key'   => 'opal-theme-license',
			'parent_slug'  => 'opal-theme',
		) );
		$cmb->add_field( array(
			'name'    => esc_html__( 'User Name Envato', 'petie' ),
			'id'      => 'username',
			'type'    => 'text',
			'default' => '',
		) );
		...
		$cmb->add_field( array(
			'name'    => esc_html__( 'Purchase Code', 'petie' ),
			'id'      => 'purchased_code',
			'type'    => 'text',
			'default' => '',
		) );
	...
	public function edit_tgmpa_notice_action_links( $action_links ) {
		...
		if ( 'opal-theme-plugins' == $current_screen->id ) {
			$link_template = '<a id="manage-plugins" class="button-primary" style="margin-top:1em;" href="#opal-install-plugins">' . esc_attr__( 'Manage Plugins Below', 'petie' ) . '</a>';
		} else {
			$link_template = '<a id="manage-plugins" class="button-primary" style="margin-top:1em;" href="' . esc_url( self_admin_url( 'admin.php?page=opal-theme-plugins' ) ) . '#opal-install-plugins">' . esc_attr__( 'Go Manage Plugins', 'petie' ) . '</a>';
		}
...
//
	...
	public function edit_admin_menus() {
		...
		if ( current_user_can( 'edit_theme_options' ) ) {
			$submenu['opal-theme'][0][0] = esc_attr__( 'Welcome', 'petie' );
		}
	}
	...
	public function custom_menu_import( $args ) {
		...
		$args['menu_title']  = esc_html__( 'Import Demos', 'petie' );
	...
	private function setup_tab_menus() {
		...
		if ( $this->get_plugins_require_count() <= 0 ) {
			$this->tabs_menu['demos'] = [
				'name' => esc_html__( 'Demo Importer', 'petie' ),
		...
		$this->tabs_menu['tutorials'] = [
			'name' => esc_html__( 'Tutorials', 'petie' ),
                ...
                <div class="row">
                    <div class="col">
                        <div class="inner theme-infomation">
                            ...
                            <h1 class="theme-title"><?php esc_attr_e( 'Welcome to', 'petie' ); ?>

								<?php echo esc_html( $my_theme->get( 'Name' ) ); ?> !
                            </h1>
                            <span class="theme-version"><?php echo esc_attr__( 'Version', 'petie' ) . ' ' . $my_theme->get( 'Version' ); ?></span>
                        </div>
                        <ul class="opal-tabs">
