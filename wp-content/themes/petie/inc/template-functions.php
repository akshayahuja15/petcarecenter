 ...
 */
if ( ! function_exists( 'petie_is_frontpage' ) ) {

	function petie_is_frontpage() {
		return ( is_front_page() || is_home() );
	}
}


if ( ! function_exists( 'petie_page_enable_breadcrumb' ) ) {
	/**
	 * @return bool
	 */
	function petie_page_enable_breadcrumb() {

		if ( ! is_page() ) {
			return true;
		}

		$check = petie_get_metabox( get_the_ID(), 'osf_enable_breadcrumb', true );
...
if ( ! function_exists( 'petie_get_query' ) ) {
...
if ( ! function_exists( 'petie_get_placeholder_image' ) ) {

	/**
	 * @return string
	 */
	function petie_get_placeholder_image() {
		return get_parent_theme_file_uri( '/assets/images/placeholder.png' );
	}

}

if (!function_exists('petie_is_elementor_activated')) {
    function petie_is_elementor_activated() {
        return function_exists('elementor_load_plugin_textdomain');
    }
}

if ( ! function_exists( 'petie_is_osf_framework_activated' ) ) {
	/**
	 * Query WooCommerce activation
	 */
	function petie_is_osf_framework_activated() {
		return class_exists( 'PetieCore' ) ? true : false;
	}
}


if ( ! function_exists( 'petie_get_metabox' ) ) {
	 ...
	 * @return bool|mixed
	 */
	function petie_get_metabox( $id, $key, $default = false ) {
...
if ( ! function_exists( 'petie_is_blog_archive' ) ) {
	function petie_is_blog_archive() {
		return ( is_home() && is_front_page() ) || is_category() || is_tag() || is_post_type_archive( 'post' );
	}
}

if ( ! function_exists( 'petie_get_excerpt' ) ) {
	function petie_get_excerpt( $excerpt_length = 55 ) {
		global $post;
...
if ( ! function_exists( 'petie_do_shortcode' ) ) {
	 ...
	 */
	function petie_do_shortcode( $tag, array $atts = array(), $content = null ) {
...
if ( ! function_exists( 'petie_nav_menu_social_icons' ) ) {
	 ...
	 */
	function petie_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
		// Get supported social icons.
		$social_icons = petie_social_links_icons();
...
add_filter( 'walker_nav_menu_start_el', 'petie_nav_menu_social_icons', 10, 4 );

if ( ! function_exists( 'petie_social_links_icons' ) ) {
	 ...
	 */
	function petie_social_links_icons() {
		...
		return apply_filters( 'petie_social_links_icons', $social_links_icons );
	}
}

if ( ! function_exists( 'petie_dropdown_icon_to_menu_link' ) ) {
	/**
	 ...
	 */
	function petie_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
...
add_filter( 'nav_menu_item_title', 'petie_dropdown_icon_to_menu_link', 10, 4 );

if ( ! function_exists( 'petie_is_header_builder' ) ) {
	/**
	 * @return bool
	 */
	function petie_is_header_builder() {
		global $osf_header;
...
if ( ! function_exists( 'petie_the_header_builder' ) ) {
	/**
	 * @return void
	 */
	function petie_the_header_builder() {
		echo petie_get_header_builder_html();
	}
}

if ( ! function_exists( 'petie_get_header_builder_html' ) ) {

	/**
	 * @return string
	 */
	function petie_get_header_builder_html() {
...
if ( ! function_exists( 'petie_is_footer_builder' ) ) {
	/**
	 * @return bool
	 */
	function petie_is_footer_builder() {
		global $osf_footer;
...
if ( ! function_exists( 'petie_the_footer_builder' ) ) {
	/**
	 * @return void
	 */
	function petie_the_footer_builder() {
		echo petie_get_footer_builder_html();
	}
}


if ( ! function_exists( 'petie_get_footer_builder_html' ) ) {
	function petie_get_footer_builder_html() {
...
if ( ! function_exists( 'petie_license_get_option' ) ) {
	function petie_license_get_option( $key = '', $default = false ) {
...
function petie_social_share() {
	if ( petie_is_osf_framework_activated() && get_theme_mod( 'osf_socials' ) ) {
		$template = WP_PLUGIN_DIR . '/petie-core/templates/socials.php';
...
function petie_social_heading() {
	return esc_html__( 'Share:', 'petie' );
}

add_filter( 'osf_social_heading', 'petie_social_heading' );

function petie_get_post_link( $taxonomy = 'category', $post_type = [ 'any' ] ) {
...
if ( ! function_exists( 'petie_fnc_related_post' ) ) {
	function petie_fnc_related_post( $relate_count = 4, $posttype = 'post', $taxonomy = 'category' ) {
		...
		if ( $related->have_posts() ) {
			...
			echo '<h3 class="related-heading">' . esc_html__( 'Maybe you like', 'petie' ) . '</h3>';
...
if ( ! function_exists( 'petie_get_review_counting' ) ) {
	function petie_get_review_counting() {
