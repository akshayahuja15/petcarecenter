...
if (!petie_page_enable_breadcrumb()) {
    return;
}
...
$otf_home = esc_html__('Home', 'petie');
$otf_blog = esc_html__('Blog', 'petie');
$otf_title = '';
                    ...
                    if (is_category()) {
                        ...
                        if (is_page()) {
                            ...
                            if (is_tag()) {
                                ...
                                $otf_title = esc_html__('Day', 'petie');
                                // Day archive

                                // Year link
                                $otf_output .= '<li class="list-inline-item item"><a href="' . esc_url(get_year_link(get_the_time('Y'))) . '" title="' . esc_attr(get_the_time('Y')) . '">' . get_the_time('Y') . esc_html__(' Archives', 'petie') . '</a></li>';
                                $otf_output .= '<li class="list-inline-item separator"> ' . $otf_sep . ' </li>';

                                // Month link
                                $otf_output .= '<li class="list-inline-item item"><a href="' . esc_url(get_month_link(get_the_time('Y'), get_the_time('m'))) . '" title="' . esc_attr(get_the_time('M')) . '">' . get_the_time('M') . esc_html__(' Archives', 'petie') . '</a></li>';
                                $otf_output .= '<li class="list-inline-item separator"> ' . $otf_sep . ' </li>';

                                // Day display
                                $otf_output .= '<li class="list-inline-item item current"> ' . get_the_time('jS') . ' ' . get_the_time('M') . esc_html__(' Archives', 'petie') . '</li>';

                            } else {
                                if (is_month()) {
                                    $otf_title = esc_html__('Month', 'petie');
                                    // Month Archive
                                    ...
                                    // Month display
                                    $otf_output .= '<li class="list-inline-item item">' . get_the_time('M') . esc_html__(' Archives', 'petie') . '</li>';

                                } else {
                                    if (is_year()) {
                                        $otf_title = esc_html__('Year', 'petie');
                                        // Display year archive
                                        $otf_output .= '<li class="list-inline-item item current">' . get_the_time('Y') . esc_html__('Archives', 'petie') . '</li>';
                                        ...
                                        if (is_author()) {
                                            ...
                                            if (!empty($author->ID)) {
                                                ...
                                                $otf_output .= '<li class="list-inline-item item home"><a href="' . esc_url(get_home_url()) . '" title="' . esc_attr($otf_home) . '">' . $otf_home . '</a></li>';
                                                $otf_output .= '<li class="list-inline-item separator"> ' . $otf_sep . ' </li>';
                                                $otf_title = esc_html__('Author', 'petie');
                                                // Get the author information
                                                $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Author: ', 'petie') . '<a href="' . get_author_posts_url($otf_userdata->ID, $otf_userdata->nice_name) . '">' . $otf_userdata->display_name . '</a></li>';
                                            }
                                            ...
                                            if (get_query_var('paged')) {
                                                ...
                                                // Paginated archives
                                                $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Page', 'petie') . ' ' . get_query_var('paged', '1') . '</li>';

                                            } else {
                                                if (is_search()) {
                                                    $otf_title = esc_html__('Search', 'petie');
                                                    // Search results page
                                                    $otf_output .= '<li class="list-inline-item item home"><a href="' . esc_url(get_home_url()) . '" title="' . esc_attr($otf_home) . '">' . $otf_home . '</a></li>';
                                                    $otf_output .= '<li class="list-inline-item separator"> ' . $otf_sep . ' </li>';
                                                    $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Keyword: ', 'petie') . get_search_query() . '</li>';

                                                } elseif (is_404()) {
                                                    $otf_title = esc_html__('Error 404', 'petie');
                                                    // 404 page
                                                    $otf_output .= '<li class="list-inline-item item home"><a href="' . esc_url(get_home_url()) . '" title="' . esc_attr($otf_home) . '">' . $otf_home . '</a></li>';
                                                    $otf_output .= '<li class="list-inline-item separator"> ' . $otf_sep . ' </li>';
                                                    $otf_output .= '<li class="list-inline-item item current">' . $otf_title . '</li>';
